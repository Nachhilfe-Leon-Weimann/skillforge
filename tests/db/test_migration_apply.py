...
_GRANT_MODE_REVISION = "0011_grant_mode_user_accounts"
_BEFORE_GRANT_MODE_REVISION = "0010_subject_title_unique"

_GRANT_MODE_ENUM_LABELS = {
...
def test_grant_mode_and_user_account_migration_is_reversible(migration_db_url: str) -> None:
    ...
    # change, and `alembic check` is blind to both - so assert the types, the key and the rows on
    # a database that holds data on either side of the revision. The empty-database path is
    # covered by test_migrations_apply_match_models_and_reverse.
    _alembic(migration_db_url, "upgrade", _BEFORE_GRANT_MODE_REVISION)
    asyncio.run(_run_on_server(migration_db_url, _SEED_BEFORE_GRANT_MODES))

    _alembic(migration_db_url, "upgrade", _GRANT_MODE_REVISION)
    for name, labels in _GRANT_MODE_ENUM_LABELS.items():
    ...
    _alembic(migration_db_url, "downgrade", _BEFORE_GRANT_MODE_REVISION)
    for name in _GRANT_MODE_ENUM_LABELS:
