...
- _Acceptance criteria:_
  - [ ] Upgrade and downgrade run clean against an empty and a seeded database; the downgrade drops the three enum
        types.
  - [ ] Inserting an e-mail with an uppercase letter violates the check constraint; two accounts for one party
