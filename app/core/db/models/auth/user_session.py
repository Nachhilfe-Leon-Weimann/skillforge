...
class UserSession(CreatedAtMixin, AuthBase):
    """One row per login (decision K). ``id`` is the token's ``sid``; only the ``application_client``
    that opened the session may refresh or revoke it. Refresh tokens are opaque, rotating and stored
