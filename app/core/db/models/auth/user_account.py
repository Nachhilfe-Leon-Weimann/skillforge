...
class UserAccount(TimestampMixin, AuthBase):
    """A login bound to exactly one person party (decision C). ``id`` is the token's ``principal_id``;
    ``party_id`` cascades on delete and ``Party`` gets no relationship back (decision M)."""
    ...
    status: Mapped[UserAccountStatus] = mapped_column(
        ...
        nullable=False,
        default=UserAccountStatus.INVITED,
    ...
    sessions: Mapped[list[UserSession]] = relationship(
        "UserSession", back_populates="user_account", cascade="all, delete-orphan"
    )
    action_tokens: Mapped[list[UserActionToken]] = relationship(
        "UserActionToken", back_populates="user_account", cascade="all, delete-orphan"
    )
